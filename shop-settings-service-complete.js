...
class ShopSettingsService {
    ...
    getDefaultSettings() {
        return {
            ...
            // API Credentials - All API keys in one place
            api: {
                // Voice AI (VAPI)
                vapi: {
                    enabled: true,
            ...
            inventory: {
                ...
                reorderMultiplier: 2.0, // 2 weeks of average usage
                optimalMultiplier: 4.0, // 4 weeks of average usage
                ...
                enableTracking: true, // Track parts usage
    ...
    maskSecrets(settings) {
        ...
        // Mask new API section credentials
        if (masked.api?.vapi?.apiKey) {
            masked.api.vapi.apiKey = this.maskApiKey(masked.api.vapi.apiKey);
        }
        if (masked.api?.vapi?.phoneId) {
            masked.api.vapi.phoneId = this.maskApiKey(masked.api.vapi.phoneId);
        }
        if (masked.api?.nexpart?.password) {
            masked.api.nexpart.password = this.maskApiKey(masked.api.nexpart.password);
        }
        if (masked.api?.licensePlate?.apiKey) {
            masked.api.licensePlate.apiKey = this.maskApiKey(masked.api.licensePlate.apiKey);
        }
        if (masked.api?.email?.sendgridApiKey) {
            masked.api.email.sendgridApiKey = this.maskApiKey(masked.api.email.sendgridApiKey);
        }
        if (masked.api?.payment?.paypal?.clientSecret) {
            masked.api.payment.paypal.clientSecret = this.maskApiKey(masked.api.payment.paypal.clientSecret);
        }
        if (masked.api?.payment?.stripe?.secretKey) {
            masked.api.payment.stripe.secretKey = this.maskApiKey(masked.api.payment.stripe.secretKey);
        }
        if (masked.api?.payment?.square?.accessToken) {
            masked.api.payment.square.accessToken = this.maskApiKey(masked.api.payment.square.accessToken);
        }
        
        // Mask social media tokens in API section
        if (masked.api?.socialMedia) {
            Object.values(masked.api.socialMedia).forEach(platform => {
                if (platform.accessToken) {
                    platform.accessToken = this.maskApiKey(platform.accessToken);
                }
                if (platform.apiKey) {
